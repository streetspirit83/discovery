/**
 * status-logic.js — Pure alert engine, shared with merkliste.
 *
 * Mirrors merkliste-app/netlify/functions/lib/status-logic.js so discovery
 * alerts use the exact same evaluation the portfolio push already uses. No DOM,
 * no Store, no side-effects — safe in the browser and (later) in the scheduled
 * ntfy function. The only discovery addition: alerts with `enabled === false`
 * are skipped (per-alert mute / unselect in the Alert-Overview).
 */
...
/* An alert counts only if it isn't explicitly disabled (per-alert mute). */
export function alertEnabled(a) {
...
/* Evaluate all alerts on a ticker respecting AND-groups and the enabled flag.
   Returns alerts array with _trig set, plus summary booleans. */
export function evaluateAlerts(rawAlerts, q) {
  const active = (rawAlerts || []).filter(alertEnabled);
...
export function computeStatus(ticker, enrichedQ) {
  ...
  const raw = ticker.user?.alerts || ticker.alerts || [];
